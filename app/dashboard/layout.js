...
import { LayoutDashboard, Users, GraduationCap, Calendar, BookOpen, Wallet, Inbox, LogOut, Menu, X } from 'lucide-react';
...
export default function DashboardLayout({ children }) {
  ...
  const menu = user.role === 'admin' ? adminMenu : teacherMenu;
      ...
      <aside className={`${open ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 fixed md:sticky top-0 left-0 z-30 w-72 h-screen bg-white border-r border-border transition-transform flex flex-col`}>
        <div className="p-4 border-b border-border flex items-center gap-3">
          ...
          <div className="leading-tight">
            ...
            <div className="text-[10px] text-muted-foreground uppercase tracking-wider">{user.role === 'admin' ? 'Admin Panel' : 'Asatidz Panel'}</div>
          </div>
        ...
        <div className="p-3 border-t border-border">
          <div className="flex items-center gap-3 mb-3 px-2">
            <div className="w-9 h-9 rounded-full bg-gradient-to-br from-sky-500 to-blue-600 flex items-center justify-center text-white font-bold text-sm">
              {user.name?.[0] || 'U'}
            </div>
